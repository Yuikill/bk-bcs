<template>
  <bk-table
    ...
    <bk-table-column :label="(() => h('span', { class: 'required' }, $t('显示名')))" prop="key" :width="144">
      <template #default="{ row }">
    ...
    <bk-table-column label="" property="delete" :width="48">
      <template #default="{ index }">
        <i class="bk-bscp-icon icon-reduce delete-icon" @click="handleDelete(index)"></i>
      </template>
    </bk-table-column>
  </bk-table>
</template>
...
<script lang="ts" setup>
  ...
  const handleDelete = (index: number) => {
    // filedsList.value.split(index, 1);
    console.log(index);
  };
