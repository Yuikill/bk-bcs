export default {
  ...
  '因为设置文件权限操作不是在业务容器中执行，而是在 Sidecar 容器中执行，因此需要在 Sidecar容器中创建相应的用户（UID）、用户组（GID）。如果无需使用容器客户端可不配置 UID 和 GID': 'Because the operation of setting file permissions is not executed in the business container, but in the Sidecar container, you need to create the corresponding user (UID), user group (GID) in the Sidecar container. If you do not need to use the container client, you can configure UID and GID',
  ...
  '下载配置文件时，保留目录层级，并将其保存到指定目录下，例如：将 /etc/nginx.conf 文件下载到 /tmp 目录时，文件保存在 /tmp/etc/nginx.conf': 'When downloading config files, preserve directory structure and save them to a specified directory. For example, downloading the file /etc/nginx.conf to the /tmp directory would save it as /tmp/etc/nginx.conf',
  '下载配置文件时，不保留目录层级，并将其保存到指定目录下，例如：将 /etc/nginx.conf 文件下载到 /tmp 目录时，文件保存在 /tmp/nginx.conf': 'When downloading config files, do not preserve directory structure and save them directly to a specified directory. For example, downloading the file /etc/nginx.conf to the /tmp directory would save it as /tmp/nginx.conf',
  ...
  '请输入配置项名称（key）用以获取对应值（value）': 'Please enter the config item name (key) to retrieve the corresponding value',
