export default {
  ...
  '因为设置文件权限操作不是在业务容器中执行，而是在 Sidecar 容器中执行，因此需要在 Sidecar容器中创建相应的用户（UID）、用户组（GID）。如果无需使用容器客户端可不配置 UID 和 GID': '因为设置文件权限操作不是在业务容器中执行，而是在 Sidecar 容器中执行，因此需要在 Sidecar容器中创建相应的用户（UID）、用户组（GID）。如果无需使用容器客户端可不配置 UID 和 GID',
